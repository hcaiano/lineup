...
enum Brand {
    ...
    /// logo stays Lineup's, so the suite still reads as one app. Hints deliberately keeps the app
    /// accent (no fourth brand colour), and unknown ids (a future tool, or a section written by a
    /// newer build) fall back to the app accent too.
    static func accent(for id: ToolID) -> NSColor {
        ...
        case .hints: return blue
        default: return blue // Any unknown id keeps the app accent.
        }
    }
