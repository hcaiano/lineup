...
enum ShortcutKit {
    ...
    static func tilesDefaults(includeShift: Bool) -> TilesSettings {
        ...
        // Leave Caps+1…4 and the user's existing Cycler bindings untouched. U/I/O/P select the
        // four Tiles workspaces in both Hyperkey modes. W/A/X/D follows the spatial layout: up,
        // left, down, right. In compact mode, physical Shift generates movement for the same
        // directional keys; when Hyperkey already includes Shift, Y/G/B/H are distinct movement
        // keys because the physical-Shift counterpart is indistinguishable from the base bind.
        let focusKeys = [kVK_ANSI_W, kVK_ANSI_A, kVK_ANSI_X, kVK_ANSI_D]
        let moveKeys = includeShift
            ? [kVK_ANSI_Y, kVK_ANSI_G, kVK_ANSI_B, kVK_ANSI_H]
            : focusKeys
        ...
        return TilesSettings(
            workspace1: binding("workspace1", kVK_ANSI_U, base),
            workspace2: binding("workspace2", kVK_ANSI_I, base),
            workspace3: binding("workspace3", kVK_ANSI_O, base),
            workspace4: binding("workspace4", kVK_ANSI_P, base),
            nextWindow: binding("nextWindow", kVK_Tab, base),
            focusTileLeft: binding("focusTileLeft", focusKeys[1], base),
            focusTileRight: binding("focusTileRight", focusKeys[3], base),
            focusTileUp: binding("focusTileUp", focusKeys[0], base),
            focusTileDown: binding("focusTileDown", focusKeys[2], base),
            moveWindowLeft: binding("moveWindowLeft", moveKeys[1], hyper),
            moveWindowRight: binding("moveWindowRight", moveKeys[3], hyper),
            moveWindowUp: binding("moveWindowUp", moveKeys[0], hyper),
            moveWindowDown: binding("moveWindowDown", moveKeys[2], hyper),
